...
public class OrdemServicoRoutes {
    ...
    public OrdemServicoRoutes() {
        ...
        // var editarUseCase = new EditarOrdemServicoUseCase(repository); // DEPRECATED
        // var atualizarStatusUseCase = new AtualizarStatusOrdemServicoUseCase(repository); // DEPRECATED
        ...
        var concluirUseCase = new ConcluirOrdemServicoUseCase(repository);
        
        this.controller = new OrdemServicoController(
            criarUseCase, buscarUseCase, listarUseCase,
            deletarUseCase, adicionarServicoUseCase, removerServicoUseCase,
            adicionarProdutoUseCase, removerProdutoUseCase, concluirUseCase
        );
    }
    ...
    public void register(Javalin app) {
        ...
        // DEPRECATED: OrdemServico não tem mais edição/status
        ...
        // PATCH /api/ordens-servico/{id}/concluir - Concluir ordem de serviço
        app.patch("/api/ordens-servico/{id}/concluir", ctx -> {
            try {
                String id = ctx.pathParam("id");
                var response = controller.concluir(id);
                ctx.status(200).json(response);
            } catch (IllegalArgumentException e) {
                ctx.status(400).json(new ErroResponse("VALIDACAO", e.getMessage()));
            } catch (Exception e) {
